...
class workingFile:  
    ...
    def show_initial_directory(self):
        ...
        fname = myfile.getOpenFileName(caption = 'Open file', directory = '\\home\\orangepi\\Desktop\\',filter = 'pnt files (*.pnt)', options=QtWidgets.QFileDialog.DontUseNativeDialog)
        if fname[0] != '':
    ...
    def save_file(self, content):
        savefilePath = myfile.getSaveFileName(caption = 'Save file', directory =  '\\home\\orangepi\\Desktop\\',filter = 'pnt files (*.pnt)', options=QtWidgets.QFileDialog.DontUseNativeDialog)
        if savefilePath[0] != '':
